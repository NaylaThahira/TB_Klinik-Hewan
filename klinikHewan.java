...
public class klinikHewan {
    ...
    try {
        ...
        String namaDokter = scanner.nextLine();
         ...
         // Mendapatkan tanggal saat ini untuk tanggal diagnosis
         LocalDate tanggalDiagnosis = LocalDate.now();

         // Membuat objek Dokter
         Dokter dokter = new Dokter(namaDokter);
        
         // Membuat objek Obat
         // Inisialisasi dengan ID Obat yang sesuai dan stok 0 (karena stok akan diperiksa di database)
         Obat obat = new Obat(namaObat);
 
         // Panggil metode diagnosa dari Dokter
         dokter.diagnosa(penyakit, dokter.getNama(), obat, dosis);
        ...
        if (rs.next()) { // Jika obat ditemukan
            ...
            if (stok >= dosis) { // Jika stok cukup
                ...
                String sqlKlinikHewan = "INSERT INTO klinkhewan (idHewan, namaHewan, jenisHewan, berat, namaPemilik, penyakit, namaObat, dosis, namaDokter, tanggalDiagnosis) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                psKlinikHewan.setDate(10, java.sql.Date.valueOf(tanggalDiagnosis));// Mengisi parameter Tanggal Diagnosis
                psKlinikHewan.executeUpdate();

                System.out.println("Data hewan dan diagnosa berhasil ditambahkanditambahkan pada tanggal:" + tanggalDiagnosis);
            } else {
